...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if not saved_files or va_loss < -saved_files[0][0]:
            save_path = args.ckpt_dir / f"backbone_ep{epoch}_ls{round(va_loss, 2)}.pth"
...
def parse_args():
    ...
    parser.add_argument('--train_image_dir', type=pathlib.Path,
                        default='hw1_data/p1_data/mini/train')
    parser.add_argument('--device', type=torch.device,
                        default='cuda:2' if torch.cuda.is_available() else 'cpu')

    parser.add_argument("--tensorboard_path",
                        type=pathlib.Path, default="./p1_results_2/board/backbone/")
    parser.add_argument("--ckpt_dir",
                        type=pathlib.Path, default="./p1_results_2/pretrained_ckpt")
    ...
    parser.add_argument("--batch_size", type=int, default=128) # 128 -> 8.4GB, 256 -> 15.9GB
